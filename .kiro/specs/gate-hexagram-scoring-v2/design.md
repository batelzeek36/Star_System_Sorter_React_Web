...
The Gate/Hexagram Star System Scoring system generates deterministic, evidence-based weights mapping 384 gate.line combinations to 8 star system archetypes. The design follows a three-phase manual analysis process: (1) score using complete texts, (2) extract evidence to justify scores, and (3) generate structured output files. This approach ensures academic rigor while maintaining consistency across all gates.
...
2. Generate _meta block with:
   ...
   - generator: "Manual" (or analyst name/identifier)
...
### Time Estimate per Gate

- **Reading sources**: ~5-10 minutes (baselines + gate text + hexagram text)
- **Analysis & scoring**: ~15-30 minutes (all 6 lines)
- **Evidence extraction**: ~5-10 minutes (quotes and justifications)
- **Validation & fixes**: ~5-10 minutes
- **Total**: ~30-60 minutes per gate

### Processing Efficiency

**Single Analysis Session per Gate:**
- Processes all 6 lines in one session
- Maintains context across lines
- More efficient than analyzing lines separately
- Ensures consistency within each gate
