...
- `claude/Full Pass/gate-{N}-full.json` - Complete Line Companion text for all 6 lines (non-padded, Human Design primary source)
- `claude/I-Ching-Full-Pass/hexagram-{NN}.json` - Complete Legge I Ching text for all 6 lines (zero-padded, cross-reference for Option A gating)

**Critical Rules**:
- Use ONLY these three files for scoring
- Process all 6 lines in a single pass
- Generate both weight and evidence files
- Treat Line Companion as the primary narrative; use Legge as the cross-reference to confirm the shared thread
...
**Objective**: Score all 8 star systems against complete source texts for lines {N}.1 through {N}.6, then select top-2 systems per line.

### Step 1: Read Complete Source Texts
For each line {N}.1 through {N}.6, read sources **in this order**:
1. **Line Companion (primary Human Design lens)** – `claude/Full Pass/gate-{N}-full.json`
2. **Legge I Ching (cross-reference)** – `claude/I-Ching-Full-Pass/hexagram-{NN}.json`

Guidance:
- HD/Line Companion is the source of record; start there to understand the behavioral axis.
- Use the Legge line to confirm, sharpen, or contrast the HD reading so you can articulate the common thread.
- Do NOT summarize or compress the texts; keep full context in working memory for scoring.
...
**Weight Calibration Scale (0.00-0.95)**:
...
**Precision Requirement**: All weights MUST be multiples of 0.01 (e.g., 0.75, 0.62, 0.48) and ≤0.95.
...
```
ERROR: Pairwise exclusion violated: {system1} and {system2} in line {N}.{L}
...
**Source**: `hexagram-{NN}.json`, same line number as gate.line
...
**Source**: `gate-{N}-full.json`, any line within the gate (line-agnostic)
...
2. If any quote >25 words:
   ```
   ERROR: Quote exceeds 25 words in {N}.{L} for {system}
   Quote: "{quote_text}"
   Word count: {count}
   ```
   **STOP. Do not proceed.**
...
**Path**: `GPT-5/star-maps/gateLine_star_map_Gate{NN}.json`
...
- **Keys**: Sorted ascending (`"{NN}.1"`, `"{NN}.2"`, ..., `"{NN}.6"`)
...
**Path**: `GPT-5/evidence/gateLine_evidence_Gate{NN}.json`
...
- **generator**: `"GPT-5"`, `"Claude"`, or `"Manual"` (must match weight file)

#### Line Entry Requirements:
- **Scope**: Only include lines with weight >0 (skip lines where all systems = 0)
- **Keys**: Sorted ascending (`"{NN}.1"`, `"{NN}.2"`, etc.)
...
- **legge1899**:
  ...
  - `locator`: `"Hex {N}, Line {L}"` (e.g., `"Hex 1, Line 2"`)
  - `attribution`: Always `"Legge 1899"`
  
- **line_companion**:
  - `quote`: Verbatim text ≤25 words (or `""` if none)
  - `locator`: `"Gate {N}, Line {L}"` (may be different line than gate.line being scored)
...
```
ERROR: Pairwise exclusion violated in line {N}.{L}
...
```
ERROR: Top-2 constraint violated in line {N}.{L}
...
- In evidence file: `"line_companion": { "quote": "", "locator": "Gate {N}, Line {L}", "attribution": "Ra Uru Hu (LC)" }`
- Reduce confidence by 1 level

**If Legge text missing for a line**:
```
ERROR: Legge text missing for Hex {N}, Line {L}
Cannot proceed (Legge cross-reference required for Option A same-line validation when weight >0.50)
```
**STOP. Do not proceed.**
...
- [ ] `generator` is `"GPT-5"`, `"Claude"`, or `"Manual"` (match the agent producing the files)
...
You are now ready to process Gate {N}.
