# Gate 25 Star System Scoring Prompt
...
- `claude/Full Pass/gate-25-full.json` - Complete Line Companion text for all 6 lines (non-padded)
- `claude/I-Ching-Full-Pass/hexagram-25.json` - Complete Legge I Ching text for all 6 lines (zero-padded)
...
- [ ] `claude/Full Pass/gate-25-full.json` (non-padded gate number)
- [ ] `claude/I-Ching-Full-Pass/hexagram-25.json` (zero-padded hexagram number)
...
**Objective**: Score all 8 star systems against complete source texts for lines 25.1 through 25.6, then select top-2 systems per line.

### Step 1: Read Complete Source Texts
For each line 25.1 through 25.6:
- Read the **COMPLETE** Line Companion text from `gate-25-full.json`
- Read the **COMPLETE** Legge I Ching text from `hexagram-25.json`
...
```
ERROR: Pairwise exclusion violated: {system1} and {system2} in line 25.{L}
...
**Source**: `hexagram-25.json`, same line number as gate.line
...
**Source**: `gate-25-full.json`, any line within the gate (line-agnostic)
...
2. If any quote >25 words:
   ```
   ERROR: Quote exceeds 25 words in 25.{L} for {system}
   Quote: "{quote_text}"
   Word count: {count}
   ```
   **STOP. Do not proceed.**
...
**Path**: `GPT-5/star-maps/gateLine_star_map_Gate25.json`
...
- **Keys**: Sorted ascending (`"25.1"`, `"25.2"`, ..., `"25.6"`)
...
**Path**: `GPT-5/evidence/gateLine_evidence_Gate25.json`
...
- **Keys**: Sorted ascending (`"25.1"`, `"25.2"`, etc.)
...
- **legge1899**:
  ...
  - `locator`: `"Hex 25, Line {L}"` (e.g., `"Hex 1, Line 2"`)
  - `attribution`: Always `"Legge 1899"`
  
- **line_companion**:
  - `quote`: Verbatim text ≤25 words (or `""` if none)
  - `locator`: `"Gate 25, Line {L}"` (may be different line than gate.line being scored)
...
```
ERROR: Pairwise exclusion violated in line 25.{L}
...
```
ERROR: Top-2 constraint violated in line 25.{L}
...
- In evidence file: `"line_companion": { "quote": "", "locator": "Gate 25, Line {L}", "attribution": "Ra Uru Hu (LC)" }`
- Reduce confidence by 1 level

**If Legge text missing for a line**:
```
ERROR: Legge text missing for Hex 25, Line {L}
...
- [ ] Run validation: `python GPT-5/scripts/validate_schemas.py --weights GPT-5/star-maps/gateLine_star_map_Gate25.json --evidence GPT-5/evidence/gateLine_evidence_Gate25.json`
...
You are now ready to process Gate 25.
