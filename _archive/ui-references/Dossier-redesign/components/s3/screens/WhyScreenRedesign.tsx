...
import { ChevronLeft, Sparkles, Orbit, GitBranch, Cpu, Target, User, Brain, ChevronDown } from 'lucide-react';
...
const mockResult: ClassificationResult = {
  ...
  contributions: [
    { source: 'channel', identifier: 'Channel 10-34', system: 'Orion', points: 15, percentage: 20.0, description: 'The Channel of Exploration • Behavior / Power' },
    { source: 'channel', identifier: 'Channel 25-51', system: 'Orion', points: 14, percentage: 18.1, description: 'The Channel of Initiation • Universal Love / Shock' },
    { source: 'gate', identifier: 'Gate 13', system: 'Orion', points: 3, percentage: 3.7, description: 'The Listener • Direction / Fellowship of Man' },
    { source: 'gate', identifier: 'Gate 18', system: 'Orion', points: 8, percentage: 10.1, description: 'Correction • Correction of Patterns' },
    { source: 'gate', identifier: 'Gate 52', system: 'Orion', points: 8, percentage: 10.0, description: 'Inaction / Stillness • The Gate of Keeping Still Mountain' },
    { source: 'gate', identifier: 'Gate 64', system: 'Orion', points: 5, percentage: 5.7, description: 'Confusion • Before Completion' },
    { source: 'center', identifier: 'Undefined Sacral', system: 'Orion', points: 6, percentage: 7.5, description: 'Open to external life force energy' },
    { source: 'center', identifier: 'Undefined Solar Plexus', system: 'Orion', points: 6, percentage: 7.5, description: 'Amplifies and samples emotional waves' },
    { source: 'type', identifier: 'Projector', system: 'Orion', points: 10, percentage: 12.5, description: 'Non-energy type designed to guide others' },
  ]
};
...
export default function WhyScreenRedesign() {
  ...
  const groupedContributions = result.contributions.reduce((acc, contribution) => {
  ...
  const totalPoints = result.contributions.reduce((sum, c) => sum + c.points, 0);
    ...
    <div className="min-h-screen bg-gradient-to-b from-[#0a0118] via-[#1a0f2e] to-[#0a0118] text-white">
        ...
        <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-violet-500/20 via-purple-500/10 to-transparent border border-violet-500/30 p-6">
          ...
          <div className="relative">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-sm text-violet-300">Primary Classification</span>
            </div>
            <h2 className="text-white mb-2">
              {result.primary}
            </h2>
            <div className="flex items-baseline gap-2 mb-4">
              <span className="text-5xl text-violet-400">{result.percentages[result.primary]}%</span>
              <span className="text-white/60">alignment</span>
            </div>
            <p className="text-white/70 text-sm">
              Based on {result.contributions.length} contributing factors from your Human Design chart
            </p>
          </div>
        </div>
      </div>
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <span className={`text-sm ${isPrimary ? 'text-white' : 'text-white/70'}`}>
                      {system.name}
                    </span>
                    {isPrimary && (
                  ...
                  <span className={`text-sm ${isPrimary ? 'text-violet-400' : 'text-white/60'}`}>
                    {system.percentage}%
                  </span>
                </div>
      ...
      <div className="relative z-10 px-4 py-6">
        <div className="grid grid-cols-3 gap-3">
          <div className="bg-white/5 backdrop-blur-sm rounded-2xl p-4 border border-white/10">
            <div className="text-2xl text-violet-400 mb-1">{result.contributions.length}</div>
          ...
          <div className="bg-white/5 backdrop-blur-sm rounded-2xl p-4 border border-white/10">
            <div className="text-2xl text-violet-400 mb-1">{Object.keys(groupedContributions).length}</div>
      ...
      <div className="relative z-10 px-4 py-4">
        <div className="flex gap-2 overflow-x-auto pb-2 -mx-4 px-4">
          ...
          {Object.entries(groupedContributions).map(([source, contributions]) => {
            ...
            return (
              ...
              >
                ...
                {SOURCE_LABELS[source as keyof typeof SOURCE_LABELS]} ({contributions.length})
              </button>
            );
          })}
        </div>
      ...
      <div className="relative z-10 px-4 py-6">
        ...
        {selectedFilter === 'all' ? (
          <Accordion type="multiple" className="space-y-3">
            {Object.entries(groupedContributions).map(([source, contributions]) => {
              const Icon = SOURCE_ICONS[source as keyof typeof SOURCE_ICONS];
              const totalCategoryPoints = contributions.reduce((sum, c) => sum + c.points, 0);
              
              return (
                <AccordionItem 
                  key={source} 
                  value={source}
                  className="border-0 bg-white/5 backdrop-blur-sm rounded-2xl overflow-hidden border border-white/10"
                >
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="text-left">
                          ...
                          <div className="text-white/60 text-xs">
                            {contributions.length} factor{contributions.length !== 1 ? 's' : ''} • {totalCategoryPoints} points
                          </div>
                        </div>
                      </div>
                    </div>
